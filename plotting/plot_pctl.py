...
#### base case
#client_tick = [""]
#method_tick  = ["base1port"]
#method_array = ["baseline"]

#client_tick = ["c0", "c1" , "c2"]
#method_tick = ["randonly", "randselect", "selectonly"]
#method_array = ["random", "random with replica-select min load", "replica-select min load"]

client_tick = ["c0", "c1" , "c2"]
#client_tick = ["s0", "s1" , "s2"]
#method_tick  = ["test_reqresp_randonly" , "test_reqresp_randselect"]
# method_tick  = ["test_batchsz1", "test_batchsz2", "test_reqresp_randselect"]
# method_array = ["replica-select with batch_size=1", "replica-select with batch_size=2", "replica-select with batch_size=4"]

# method_tick  = [ "test_batchsz1", "test_reqresp_randselect"]
# method_array = [ "replica-select with batch_size=1", "replica-select with batch_size=4"]

# method_tick  = [ "test_allruns_randonly" , "test_allruns_randselect"]
method_tick  = ["zoomin_rates_randonly", "zoomin_rates_randselect"]
method_array = ["random", "replica-select with gossip=25us, delta=1"]
rate_tick =[ "20k", "21k", "22k", "23k", "24k", 
            "25k", "26k", "27k", "28k", "29k", 
            "30k", "31k", "32k", "33k", "34k", 
            "35k", "36k"]
...
for method in method_tick:
    mean_array = np.zeros(len(rate_tick))
    pct95th_array = np.zeros(len(rate_tick))
    pct99th_array = np.zeros(len(rate_tick))
    pct99th9_array = np.zeros(len(rate_tick))
    loss_rate = np.zeros(len(rate_tick))
    ...
    for rate in rate_tick:
        ...
        filename0 = parent_dir + working_dir + "/"+ method + "_"+ client_tick[0] + "_" + rate + "." + extension
        filename1 = parent_dir + working_dir + "/"+ method + "_"+ client_tick[1] + "_" + rate + "." + extension
        filename2 = parent_dir + working_dir + "/"+ method + "_"+ client_tick[2] + "_" + rate + "." + extension        
    ...
    ax.plot(rate_tick, mean_array, color=color_list[method_index], linestyle='solid',
        label=method_array[method_index]+":mean", marker='.', lw=1.5)
    ax.plot(rate_tick, pct95th_array, color=color_list[method_index], linestyle='dashed',
        label=method_array[method_index]+":95th pct", marker='.', lw=1.5)
    ax.plot(rate_tick, pct99th_array, color=color_list[method_index], linestyle='dotted',
        label=method_array[method_index]+":99th pct", marker='.', lw=1.5)
    ax.plot(rate_tick, pct99th9_array, color=color_list[method_index], linestyle='dashdot',
