...
mean_per_delta_mean_array = np.zeros(len(redir_tick))
mean_per_delta_sem_array = np.zeros(len(redir_tick))

pct95th_per_delta_mean_array = np.zeros(len(redir_tick))
pct95th_per_delta_sem_array = np.zeros(len(redir_tick))

pct99th_per_delta_mean_array = np.zeros(len(redir_tick))
pct99th_per_delta_sem_array = np.zeros(len(redir_tick))

pct99th9_per_delta_mean_array = np.zeros(len(redir_tick))
pct99th9_per_delta_sem_array = np.zeros(len(redir_tick))
#loss_rate = np.zeros(len(delta_tick))

method_index=0
delat_index=0
#for method in method_tick:
for redir in redir_tick:
    ...
    for run in run_tick:
        ...
        filename_prefix = method_tick[method_index] + "_r_" + redir + "_run" + run
    ...
    print( "method:" + method_tick[method_index] + ",rate:" + rate + ",redir:" + redir)
...
ax.errorbar(redir_tick, mean_per_delta_mean_array, yerr=mean_per_delta_sem_array,color=color_list[method_index], linestyle='solid',
...
ax.errorbar(redir_tick, pct95th_per_delta_mean_array, yerr=pct95th_per_delta_sem_array ,color=color_list[method_index+1], linestyle='dashed',
...
ax.errorbar(redir_tick, pct99th_per_delta_mean_array, yerr=pct99th_per_delta_sem_array ,color=color_list[method_index+2], linestyle='dotted',
...
ax.errorbar(redir_tick, pct99th9_per_delta_mean_array, yerr=pct99th9_per_delta_sem_array ,color=color_list[method_index+3], linestyle='dashdot',
