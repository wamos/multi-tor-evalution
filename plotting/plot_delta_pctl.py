...
for method in method_tick:
    mean_array = np.zeros(len(delta_tick))
    pct95th_array = np.zeros(len(delta_tick))
    pct99th_array = np.zeros(len(delta_tick))
    pct99th9_array = np.zeros(len(delta_tick))
    ...
    for delta in delta_tick:
        ...
        filename0 = parent_dir + working_dir + "/"+ method + "_d_" + delta + "_"+ client_tick[0] + "_" + rate + "." + extension
        filename1 = parent_dir + working_dir + "/"+ method + "_d_" + delta + "_"+ client_tick[1] + "_" + rate + "." + extension
        filename2 = parent_dir + working_dir + "/"+ method + "_d_" + delta + "_"+ client_tick[2] + "_" + rate + "." + extension        
        ...
        print( "method:" + method + ",rate:" + rate + ",delta:" + delta)
    ...
    ax.plot(delta_tick, mean_array, color=color_list[method_index], linestyle='solid',
        label=method_array[method_index]+":mean", marker='.', lw=1.5)
    ax.plot(delta_tick, pct95th_array, color=color_list[method_index], linestyle='dashed',
        label=method_array[method_index]+":95th pct", marker='.', lw=1.5)
    ax.plot(delta_tick, pct99th_array, color=color_list[method_index], linestyle='dotted',
        label=method_array[method_index]+":99th pct", marker='.', lw=1.5)
    ax.plot(delta_tick, pct99th9_array, color=color_list[method_index], linestyle='dashdot',
